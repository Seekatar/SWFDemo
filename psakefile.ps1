...
task IntegrationTest {
    exec -workingDirectory $sourceDir/IntegrationTest {
        dotnet test
    }
}
...
task BuildMessage {
    if ($messageVersion) {
        exec -workingDirectory $sourceDir {
            ...
            if ($folder -and $folder.Count -eq 1) {
                ...
                if (!(Test-Path $packageFolder)) {
                    Write-Warning "BuildMessages task selected, but '$packageFolder' not found. Creating."
                    $null = New-Item $packageFolder -ItemType Directory
                }
                $nugetOutput = dotnet nuget list source
                if ($nugetOutput | Select-String $packageFolder -SimpleMatch) {
                    "Found local nuget for $packageFolder"
                } else {
                    if ($nugetOutput | Select-String "\s$($folder.Name)\s") {
                        Write-Warning "nuget source $($folder.Name) exists in different folder."
                        Write-Warning "To build into this one remove the other:"
                        Write-Warning "   dotnet nuget remove source $($folder.Name)"
                        throw "$($folder.Name) nuget source exist in different folder"
                    } else {
                ...
                exec -workingDirectory $folder[0] {
                    dotnet build --configuration Release -p:Version=$messageVersion -p:AssemblyVersion=$messageVersion
                }
                exec -workingDirectory $folder[0] {
                    dotnet pack --configuration Release --output $packageFolder -p:Version=$messageVersion -p:AssemblyVersion=$messageVersion --include-source --no-build
                }
                # update all the other projects
                Get-ChildItem *.csproj -r -Exclude "*$($folder.Name).*" | ForEach-Object {
                    exec -workingDirectory (Split-Path $_ -Parent) {
                        dotnet add $_ package $folder.Name
                    }
                }
            } else {
        ...
        Write-Error "Must supply MessageVersion when using BuildMessage."
    }
