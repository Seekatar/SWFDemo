...
.PARAMETER DockerTag
Optional tag for the docker container, leave blank for ccc-cas-workflow2-service:MM.dd.yy
...
param(
    ...
    [string] $ContainerName = "workflow2-service",
    [string] $DockerTag = "ccc-cas-workflow2-service:$((Get-Date).ToString('MM.dd.yy'))",
...
Invoke-Psake -buildFile '.\psakefile.ps1' -taskList @($Task) -parameters @{
    ...
    runFolder = 'CCC.CAS.Workflow2Service'
