...
COPY ./CCC.CAS.Workflow2Service/CCC.CAS.Workflow2Service.csproj CCC.CAS.Workflow2Service/
...
WORKDIR /src/CCC.CAS.Workflow2Service
RUN dotnet restore

COPY . .

RUN export PATH="$PATH:/root/.dotnet/tools" && \
     dotnet sonarscanner begin /k:"CAS-Workflow2Service" /d:sonar.host.url="https://sonarqube.cccis.com" /d:sonar.login=$sonarQubeToken /v:$version; \
     exit 0

RUN dotnet publish "CCC.CAS.Workflow2Service/CCC.CAS.Workflow2Service.csproj" -c Release -o /app -p:Version=$version,AssemblyVersion=$version
...
ENTRYPOINT ["dotnet", "CCC.CAS.Workflow2Service.dll"]
